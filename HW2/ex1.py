...
def mean_filter(img, filter_size=3):
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            ...
            smoothed_img[i, j] = np.sum(patch * kernel)
...
if __name__ == '__main__':
    ...
    img_groundtruth = read_img(img_gt)
    filter_size = 3

    # Mean filter
    #mean_smoothed_img = mean_filter(img, filter_size)
    #show_res(img, mean_smoothed_img)
    #output_path = "./ex1_images/mean_smoothed_img.png"
    #cv2.imwrite(output_path, mean_smoothed_img)
    # print('PSNR score of mean filter: ', psnr(img, mean_smoothed_img))
    #print('PSNR score of mean filter: ', psnr(img_groundtruth, mean_smoothed_img))


    # Median filter
    #median_smoothed_img = median_filter(img, filter_size)
    #show_res(img, median_smoothed_img)
    # output_path = "./ex1_images/median_smoothed_img.png"
    # cv2.imwrite(output_path, mean_smoothed_img)
    # print('PSNR score of median filter: ', psnr(img, median_smoothed_img))
    # print('PSNR score of median filter: ', psnr(img_groundtruth, median_smoothed_img))
